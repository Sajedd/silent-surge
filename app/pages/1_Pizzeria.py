...
st.title("🍕 Analyse - Pizzeria")

# Chargement des données
df = load_data("pizzeria")
...
sns.barplot(x=zone_counts.values, y=zone_counts.index, ax=ax2, palette="magma")
...
saison_pred = st.selectbox("Saison prédiction", df["saison"].unique(), key="saison_pred")
zone_pred = st.selectbox("Zone prédiction", df["zone"].unique(), key="zone_pred")

if st.button("Prédire le best-seller 🍕"):
    produit = predict_best_seller("pizzeria", saison_pred, zone_pred)
